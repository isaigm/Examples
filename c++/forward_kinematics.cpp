...
public:
    ...
    void goTo(sf::Vector2f target)
    {
        ...
        float vv1 = std::abs(robotAngle - angleBetween - (dir - 2 * M_PI));
        float vv2 = std::abs(robotAngle + angleBetween - dir);
        std::cout << dist << "\n";
        if (angleBetween < 1.0f)
        {
            if (std::abs(dir - robotAngle) < 0.1f)
            {
                forward();
            }
    ...
    void update(float dt)
    {
        ...
        if (robotAngle < 0) robotAngle += 2 * M_PI;
        if (robotAngle > 2 * M_PI) robotAngle = 0;
