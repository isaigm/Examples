...
const static int dino_coords[][4] = { {846, 0, 43, 48},
	{937, 0, 43, 48}, 
	{980, 0, 43, 48} };
...
{
	sf::IntRect rect(dino_coords[idx][0], dino_coords[idx][1], dino_coords[idx][2], dino_coords[idx][3]);
...
int main()
{
	...
	while (window.isOpen())
	{
		...
		temp_x += dt * dino.speed;
		dino.update(dt);
		view.move(dt * dino.speed, 0);
